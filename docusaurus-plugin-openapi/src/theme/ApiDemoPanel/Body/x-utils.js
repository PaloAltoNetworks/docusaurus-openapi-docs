  ...
  if (type === "object") {
    ...
    for (let propName in props) {
      ...
      if (props[propName].xml.attribute) {
        let enumAttrVal =
          Array.isArray(props[propName].enum) && props[propName].enum[0];
        ...
        if (
          props[propName].example === undefined &&
...
export function createXMLExample(_schema, config) {
  const schema = produce(_schema, (draft) => {
    if (draft.xml === undefined) {
      draft.xml = {};
    }

    if (draft.xml.name === undefined) {
      draft.xml.name = "";
    }
  });
