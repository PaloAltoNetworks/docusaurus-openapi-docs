    ...
    switch (request.body.mode) {
      case "raw":
        ...
        if (
          ...
          try {
            ...
            return `payload = json.dumps(${pythonify(
              jsonBody,
              indentation.length
            )})\n`;
          } catch (error) {
      ...
      case "graphql":
        ...
        let query = request.body[request.body.mode].query,
          graphqlVariables;
        try {
          graphqlVariables = JSON.parse(
            request.body[request.body.mode].variables
          );
        } catch (e) {
          graphqlVariables = {};
        }
        requestBody += `payload=${sanitize(
          JSON.stringify({
            query: query,
      ...
      case "urlencoded":
        ...
        if (!_.isEmpty(enabledBodyList)) {
          bodyDataMap = _.map(enabledBodyList, function (value) {
            return (
              `${sanitize(value.key, request.body.mode, bodyTrim)}=` +
      ...
      case "formdata":
        ...
        if (!_.isEmpty(enabledBodyList)) {
          bodyDataMap = _.map(
            _.filter(enabledBodyList, { type: "text" }),
            function (value) {
              return (
                `'${sanitize(value.key, request.body.mode, bodyTrim)}': ` +
                `'${sanitize(value.value, request.body.mode, bodyTrim)}'`
          ...
          bodyFileMap = _.map(
            _.filter(enabledBodyList, { type: "file" }),
            function (value) {
