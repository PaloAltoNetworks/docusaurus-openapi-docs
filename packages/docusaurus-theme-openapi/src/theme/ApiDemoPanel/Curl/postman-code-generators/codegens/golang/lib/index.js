...
function parseRawBody(body, trim) {
  var bodySnippet;
  bodySnippet = `payload := strings.NewReader(\`${sanitizeMultiline(
    body.toString(),
    trim
  )}\`)`;
  return bodySnippet;
...
function parseGraphQL(body, trim) {
  ...
  bodySnippet = `payload := strings.NewReader("${sanitize(
    JSON.stringify({
      query: query || "",
...
function parseURLEncodedBody(body, trim) {
  ...
  payload = _.reduce(
    ...
    function (accumulator, data) {
      if (!data.disabled) {
        accumulator.push(
          `${encodeURIComponent(data.key, trim)}=${encodeURIComponent(
            data.value,
            trim
          )}`
        );
...
function parseFormData(body, trim, indent) {
  ...
  _.forEach(body, function (data, index) {
    if (!data.disabled) {
      if (data.type === "file") {
        isFile = true;
        bodySnippet += `${indent}file, errFile${index + 1} := os.Open("${
          data.src
        }")\n`;
        bodySnippet += `${indent}defer file.Close()\n`;
        bodySnippet +=
          `${indent}part${index + 1},
         errFile${index + 1} := writer.CreateFormFile("${sanitize(
            data.key,
            trim
            // eslint-disable-next-line no-useless-concat
          )}",` + `filepath.Base("${data.src}"))\n`;
        bodySnippet += `${indent}_, errFile${index + 1} = io.Copy(part${
          index + 1
        }, file)\n`;
        bodySnippet +=
          `${indent}if errFile${index + 1} != nil {` +
        ...
        bodySnippet += `${indent}mimeHeader${
          index + 1
        }["Content-Disposition"] = `;
        bodySnippet += `append(mimeHeader${
          index + 1
        ...
        bodySnippet += `mimeHeader${index + 1}["Content-Type"], "${
          data.contentType
        }")\n`;
        bodySnippet += `${indent}fieldWriter${
          index + 1
        }, _ := writer.CreatePart(mimeHeader${index + 1})\n`;
        bodySnippet += `${indent}fieldWriter${
          index + 1
        }.Write([]byte("${sanitize(data.value, trim)}"))\n\n`;
      } else {
        bodySnippet += `${indent}_ = writer.WriteField("${sanitize(
          data.key,
          trim
        )}",`;
...
function parseHeaders(headers, indent) {
  ...
  if (!_.isEmpty(headers)) {
    ...
    _.forEach(headers, function (header) {
      headerSnippet += `${indent}req.Header.Add("${sanitize(
        header.key,
        true
...
self = module.exports = {
  convert: function (request, options, callback) {
    ...
    if (timeout > 0) {
      codeSnippet += `${indent}timeout := time.Duration(${
        timeout / 1000
      } * time.Second)\n`;
    }
  ...
  getOptions: function () {
    return [
      ...
      },
      {
        ...
        default: 0,
        description:
          "Set number of milliseconds the request should wait for a " +
